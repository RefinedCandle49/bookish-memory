from Controller.Controller_Factura import *
...
class ArregloFactura:
    ...
    def buscarFactura(self, nDocumentoVenta):
        for i in range(self.tamañoFactura()):
            if nDocumentoVenta == self.dataFactura[i].getNumeroDocumento():
                return i
        return -1
    ...
    def nroSerie(self):
        self.series = []
        for i in range(self.tamañoFactura()):
            self.series.append(self.dataFactura[i].getNumeroDocumento())
