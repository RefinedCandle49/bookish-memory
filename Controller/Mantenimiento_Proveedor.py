...
class ArregloProveedor:
    ...
    def cargar(self):
        archivo = open("Model/Proveedor.txt", "r", encoding = "utf-8")
        for linea in archivo.readlines():
            ...
            dir = columna[3]
            cat = columna[4].strip()
            objPro = Proveedor(dni, raz, tel, dir, cat)
            self.adicionaProveedor(objPro)
        archivo.close()

    def grabar(self):
        archivo = open("Model/Proveedor.txt", "w+", encoding= "utf-8")
        for i in range(self.tamañoArregloProveedor()):
            archivo.write(str(self.devolverProveedor(i).getDniProveedor()) + "," + str(self.devolverProveedor(i).getRazonSocial()) + "," + str(self.devolverProveedor(i).getTelefono()) + "," + str(self.devolverProveedor(i).getDireccion()) + "," + str(self.devolverProveedor(i).getCategoria()) + "\n")
        archivo.close()
